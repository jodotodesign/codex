...
class ACF_Opening_Hours_Plugin
{
    ...
    private bool $acfAvailable;

    public function __construct()
    {
        $this->acfAvailable = function_exists('acf_add_local_field_group');

        add_action('acf/init', [$this, 'register_fields']);
        add_action('admin_notices', [$this, 'render_acf_missing_notice']);
        add_shortcode('opening_status', [$this, 'render_shortcode']);
    }

    public function register_fields(): void
    {
        if (!$this->acfAvailable) {
            return;
        }
    ...
    public function render_acf_missing_notice(): void
    {
        if ($this->acfAvailable || !current_user_can('manage_options')) {
            return;
        }
        ...
        printf(
            ...
            esc_html__(
                'Das Plugin "ACF Öffnungszeiten & Feiertage" benötigt Advanced Custom Fields (Pro oder Free), um die Optionsseite anzuzeigen.',
                'acf-opening-hours'
            )
        );
    }
